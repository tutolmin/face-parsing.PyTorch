...
def upsampling(x, shape, interpolation="nearest"):
    def resize_func(t):
        return tf.image.resize(
            t, shape,
            method=interpolation,  # 'nearest' или 'bilinear'
            align_corners=True
        )

    return Lambda(
        resize_func,
        output_shape=(shape[0], shape[1], x.shape[-1])  # (H, W, C)
    )(x)
...
pth_model = BiSeNet(n_classes=19)
pth_model.load_state_dict(torch.load("res/cp_32/99999_iter.pth"))
pth_model.eval()  # Режим инференса

keras_model = BiSeNet_keras(input_resolution=512)  # Размер должен совпадать с PyTorch-моделью


def transfer_conv_weights(pth_conv, keras_conv):
    # Получаем веса из PyTorch
    pth_weight = pth_conv.weight.detach().numpy()
    # Транспонирование: (C_out, C_in, H, W) -> (H, W, C_in, C_out)
    keras_weight = np.transpose(pth_weight, (2, 3, 1, 0))
    # Устанавливаем веса в Keras
    keras_conv.set_weights([keras_weight])

# Пример переноса для первого слоя
transfer_conv_weights(
    pth_model.cp.resnet.conv1,  # PyTorch-слой
    keras_model.get_layer("cp.resnet.conv1")  # Keras-слой
)

def transfer_bn_weights(pth_bn, keras_bn):
    gamma = pth_bn.weight.detach().numpy()
    beta = pth_bn.bias.detach().numpy()
    mean = pth_bn.running_mean.detach().numpy()
    var = pth_bn.running_var.detach().numpy()
    keras_bn.set_weights([gamma, beta, mean, var])

transfer_bn_weights(
    pth_model.cp.resnet.bn1,
    keras_model.get_layer("cp.resnet.bn1")
)
